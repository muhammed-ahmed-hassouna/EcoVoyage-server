...
const AccommodationModel = require("../../Models/accommodation/accommodation_model");
...
const getAccommodationsByID = async (req, res) => {
  ...
  try {
    const result = await AccommodationModel.getAccommodationsByID(
      accommodation_id
    );
    res.json(result);
  } catch (err) {
...
const updateAccommodation = async (req, res) => {
  ...
  try {
    ...
    const result = await AccommodationModel.updateAccommodation(
      accommodation_id,
      accommodationData
    );
...
const markAccommodationAsDeleted = async (req, res) => {
  const accommodation_id = req.params.id;
  try {
    const result = await AccommodationModel.markAccommodationAsDeleted(
      accommodation_id
    );
...
const addCommentAccomm = async (req, res) => {
  ...
  try {
    const commentResult = await AccommodationModel.addComment(
      accommodation_id,
      user_id,
      comment_text
    );
...
const getAccommodationsWithComments = async (req, res) => {
  const accommodation_id = req.params.id;
  try {
    const result = await AccommodationModel.getAccommodationsWithComments(
      accommodation_id
    );
    res.json(result);
  } catch (err) {
...
const bookAccommodation = async (req, res) => {
  const accommodation_id = req.params.id;
  const { cost, address, phone, adults, children, date_from, date_to } =
    req.body;
  const user_id = req.user.user_id;

  try {
    const result = await AccommodationModel.bookAccommodation(
      cost,
      accommodation_id,
      user_id,
      address,
...
const getBookAccommodations = async (req, res) => {
  const accommodation_id = req.params.id;
  try {
    const result = await AccommodationModel.getBookAccommodations(
      accommodation_id
    );
    ...
    if (!result.length) {
      return res
        .status(404)
        .json({ error: "No Books In this Accommodation !" });
    } else {
      res.json(result);
    }
  } catch (err) {
...
const getAccommodationsPaginated = async (req, res) => {
  try {
    ...
    const result = await AccommodationModel.getAccommodationsPaginated(
      page,
      pageSize,
      search
    );
