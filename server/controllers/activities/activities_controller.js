...
const activityModel = require("../../Models/activity/activity_model");
...
const updateActivities = async (req, res) => {
  try {
    ...
    const result = await activityModel.updateActivities(
      activities_id,
      activitiesData
    );
...
const addCommentToAc = async (req, res) => {
  ...
  try {
    const commentResult = await activityModel.addCommentToAc(
      activities_id,
      user_id,
      comment_text
    );
    res
      .status(200)
      .json({ message: "Comment added successfully", commentResult });
  } catch (err) {
    console.error(err);
    res.status(500).send("Internal Server Error");
  }
...
const getActivitiesPaginated = async (req, res) => {
  try {
    ...
    const result = await activityModel.getActivitiesPaginated(
      page,
      pageSize,
      search
    );
...
const BookActivity = async (req, res) => {
  ...
  try {
    const result = await activityModel.BookActivity(
      activities_id,
      cost,
      user_id,
      address,
