...
const getContactById = async (req, res) => {
  try {
    ...
    const result = await db.query(
      "SELECT * FROM contact_us WHERE contact_id = $1 AND is_shown = true",
      [contactId]
    );
...
const updateContactShownStatus = async (req, res) => {
  try {
    ...
    const selectResult = await db.query(
      "SELECT * FROM contact_us WHERE contact_id = $1",
      [contactId]
    );
    ...
    const updateResult = await db.query(
      "UPDATE contact_us SET is_shown = $1 WHERE contact_id = $2",
      [newStatus, contactId]
    );
    ...
    res
      .status(200)
      .json({ message: "Contact status toggled successfully.", newStatus });
  } catch (error) {
    console.error("Error toggling contact status:", error);
    res
      .status(500)
      .json({ error: "An error occurred while toggling contact status." });
  }
};
