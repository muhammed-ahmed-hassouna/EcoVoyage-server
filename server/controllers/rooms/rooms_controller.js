const roomModel = require("../../Models/rooms/rooms_model");
...
const getFilteredRooms = async (req, res) => {
  ...
  try {
    const result = await roomModel.getFilteredRooms(
      accommodation_id,
      date_from,
      date_to,
      adults,
...
const BookRoom = async (req, res) => {
  try {
    ...
    const { room_id, accommodation_id, date_from, date_to, adults, children } =
      req.body;

    // Call the BookRoom function to book the room
    const bookingResult = await roomModel.BookRoom(
      user_id,
      room_id,
      accommodation_id,
      date_from,
    ...
    res
      .status(200)
      .json({ message: "Room booked successfully", bookingResult });
  } catch (err) {
    // Handle errors
    console.error(err);
    res
