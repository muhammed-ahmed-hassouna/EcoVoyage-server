...
const addRoom = async (roomData) => {
  try {
    return await db("rooms").insert(roomData).returning("*");
  } catch (err) {
    console.error(err);
    ...
    const result = await db("rooms")
      ...
      .whereNotExists(function () {
        this.select("*")
          ...
          .andWhere(function () {
            this.where("room_booking2.date_to", ">=", date_from).andWhere(
              "room_booking2.date_from",
              "<=",
              date_to
            );
          });
...
const BookRoom = async (
  ...
  try {
    const isRoomAvailable = await getFilteredRooms(
      accommodation_id,
      date_from,
      date_to,
      adults,
