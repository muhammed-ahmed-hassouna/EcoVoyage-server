...
const getAccommodations = async () => {
  try {
    return await db.select("*").from("accommodation").where({
      is_deleted: false,
    });
  } catch (err) {
    console.error(err);
...
const getAccommodationsByID = async (accommodation_id) => {
  try {
    return await db("accommodation").select("*").where({
      is_deleted: false,
...
const addAccommodation = async (accommodationData) => {
  try {
    return await db("accommodation").insert(accommodationData).returning("*");
  } catch (err) {
    console.error(err);
...
const addComment = async (accommodation_id, user_id, comment_text) => {
  try {
    ...
    const userResult = await db("users").where({ user_id: user_id }).first();
